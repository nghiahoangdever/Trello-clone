...
const createNew = async (req, res, next) => {
  /**
   * Note (video 52): Mặc định không cần phải custom message ở phía BE làm gì vì để cho Front-end tự validate và custom message phía FE cho đẹp.
  ...
  const correctCondition = Joi.object({
    title: Joi.string().required().min(3).max(50).trim().strict().messages({
      'any.required': 'Title is required (nghiahoangdev)',
      'string.empty': 'Title is not allowed to be empty (nghiahoangdev)',
      'string.min': 'Title length must be at least 3 characters long (nghiahoangdev)',
      'string.max': 'Title length must be less than or equal to 5 characters long (nghiahoangdev)',
      'string.trim': 'Title must not have leading or trailing whitespace (nghiahoangdev)'
    }),
